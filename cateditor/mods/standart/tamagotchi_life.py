...
def register_plugin(app):
    ...
    def trigger_random_event():
        ...
        events = [
            ...
            ("Посылка", "Доставлена коробка с рыбой!\n+1 Супер-Сёмга в инвентарь.", "free_loot"),
    ...
    # --- ФУНКЦИИ ЖИЗНИ И СОХРАНЕНИЯ (Интеграция с cat_storage) ---
     # --- СИСТЕМА СОХРАНЕНИЯ И ЗАГРУЗКИ ПОД ФОРМАТ .CATLIVE ---
    def save_game():
        ...
        try:
            ...
            data = {
                ...
                "inv": app.cat_inventory, # Для обратной совместимости
            ...
            # Сохраняем в родном формате .catlive
    ...
    def recharge_power_grid():
        if app.cat_coins < 15:
            messagebox.showerror("Энергосеть", "Не хватает 15 монет!")
            return
        app.cat_coins -= 15
        app.stat_power = 100
        update_stats_ui()
    ...
    # Автоматическая загрузка сейва при старте мода
    # --- ИНВЕНТАРЬ ---
    def open_inventory():
        ...
        def use_item(name):
            if name in ["Рыбка", "Супер-Сёмга"]:
                app.stat_hunger = min(100, app.stat_hunger + (35 if name == "Рыбка" else 70))
                app.cat_inventory.remove(name)
            elif name == "Молоко":
                ...
                messagebox.showinfo("Инвентарь", f"{name} активирован и работает в фоне!")
                return
            update_stats_ui()
            save_game()
            inv_win.destroy()
    ...
    def open_cheat_console():
        ...
        def apply_cheat():
            ...
            if cheat == "catcoins":
                ...
                app.blackout_active = False
                if hasattr(app, 'apply_power_theme'):
                    app.apply_power_theme(False)
                if hasattr(app, 'render_cat'):
                    app.render_cat()
                elif hasattr(app, 'draw_cat'):
                    app.draw_cat()
    ...
    # Обертка для базовых кнопок GUI ядра для поддержки системы автосохранения
    # Обертка для базовых кнопок GUI ядра для поддержки системы автосохранения
    ...
    # --- БАЗОВЫЕ ДЕЙСТВИЯ (ИНТЕГРАЦИЯ С КНОПКАМИ) ---
    def feed_cat_realtime():
        app.stat_hunger = min(100, app.stat_hunger + 30)
        app.stat_poop = min(100, app.stat_poop + 10)
        update_stats_ui()
    ...
    def open_shop():
        ...
        items = {
            ...
            "🛋️ Мебель и Уют": [("Мягкая лежанка", 150, "build_energy"), ("Кошачий комплекс", 250, "build_fun")],
            "⚔️ Одежда и Оружие": [("Очки PRO", 100, "item_wear"), ("Лазерная указка", 180, "item_weapon")]
        }
        ...
        def buy_item(name, price, item_type):
            ...
            if item_type == "elec_solar":
                ...
                messagebox.showinfo("Магазин", "Солнечная панель установлена на крыше!")
            else:
        ...
        for cat, items_list in items.items():
            ...
            for name, price, itype in items_list:
                ...
                tk.Button(shop_win, text=btn_text, bg="#444444", fg="white", command=lambda n=name, p=price, t=itype: buy_item(n, p, t)).pack(fill="x", padx=15, pady=2)
    ...
    btns = [
        ...
        ("Охота", "#252", lambda: app.start_hunt_game() if hasattr(app, 'start_hunt_game') else messagebox.showinfo("Охота", "Кот ушёл ловить мышей!")), # Вернули охоту!
    ...
    # Привязка консоли читов на горячую клавишу ~ (тильда)
